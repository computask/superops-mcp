...
interface FetchErrorDiagnostic { stage: "fetchPage"; page: number; errorType: "rateLimit" | "server" | "graphql" | "network"; message: string; retryable: boolean; attempts: number }
interface RetryDiagnostics { retries: number; retryDelaysMs: number[] }
export interface TicketPaginationDiagnostics { pagesFetched: number; pageSize: number; apiTotalCount?: number; recordsExamined: number; recordsMatched: number; recordsReturned: number; duplicateRecordsRemoved: number; complete: boolean; truncated: boolean; nextPage: number | null; stopReason: "crossedCreatedFromBoundary" | "hasMoreFalse" | "emptyPage" | "maxPagesReached" | "maxRecordsReached" | "repeatedPageLoop" | "fetchError" }
