...
`WriteStarted`, `ResolutionWriteStarted`, and `NoteWriteStarted` are acknowledged durably before the outbound mutation. A reliable accepted response is checkpointed immediately as `FieldsUpdated`, `ResolutionWriteSucceeded`, or `NoteAdded`; a created note ID is stored with `NoteAdded`. If acknowledgement or response delivery is uncertain after mutation start, the item remains possible-write/ambiguous and is reconciled by read-back. It is never blindly replayed. A reliable response that conclusively proves non-acceptance is checkpointed as `Rejected` with `writeAttempted=true` and `writeMayHaveSucceeded=false` before any continuation is scheduled. A later invocation re-reads and revalidates before the single bounded retry. Timeout, network, HTTP 5xx, and lost-response failures remain ambiguous and are never replayed merely because an immediate read still shows the old state. Private-note retries additionally deduplicate against existing private notes before writing.
