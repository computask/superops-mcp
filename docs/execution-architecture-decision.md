...
`WriteStarted`, `ResolutionWriteStarted`, and `NoteWriteStarted` are acknowledged durably before the outbound mutation. A reliable accepted response is checkpointed immediately as `FieldsUpdated`, `ResolutionWriteSucceeded`, or `NoteAdded`; a created note ID is stored with `NoteAdded`. If acknowledgement or response delivery is uncertain after mutation start, the item remains possible-write/ambiguous and is reconciled by read-back. It is never blindly replayed. A conclusive HTTP 429 or structured GraphQL throttle rejection before private-note acceptance preserves the conservative `NoteWriteStarted` write truth while recording `Rejected` and a bounded next-eligibility time. A later invocation must re-read, revalidate, and dedupe before the single bounded private-note retry; network, timeout, 5xx, and lost-response note failures are never rescheduled as retryable writes.
