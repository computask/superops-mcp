...
`superops_operations_get` and `superops_operations_results` are read-only and owner-scoped. New Durable Object records are partitioned by owner hash, and each owner instance maintains an idempotent compact recent-operation index capped at 50 entries; the public recent list returns at most 20 fixed-field redacted summaries. Missing, expired, deleted, malformed, or wrong-owner records are excluded. Existing operation-derived records remain readable by ID for compatibility. There is no public resume or cancel tool.

Direct OAuth ownership remains the stable hash of the authenticated OAuth user. Gateway ownership is derived at the Worker boundary from the normalized SuperOps tenant context plus a non-reversible fingerprint of the presented API credential; raw credentials and bearer values are never placed in the audit context, ledger, index, or public result. Owner-scoped tools fail closed when the Worker has neither a direct OAuth user nor a complete authenticated gateway/env identity. Internal continuation remains separately authenticated by `SUPEROPS_INTERNAL_CONTINUATION_TOKEN` and uses the owner hash already persisted with the operation.
...
`WriteStarted`, `ResolutionWriteStarted`, and `NoteWriteStarted` are acknowledged durably before the outbound mutation. A reliable accepted response is checkpointed immediately as `FieldsUpdated`, `ResolutionWriteSucceeded`, or `NoteAdded`; a created note ID is stored with `NoteAdded`. If acknowledgement or response delivery is uncertain after mutation start, the item remains possible-write/ambiguous and is reconciled by read-back. It is never blindly replayed. A reliable response that conclusively proves non-acceptance is checkpointed as `Rejected` with `writeAttempted=true` and `writeMayHaveSucceeded=false` before any continuation is scheduled. For update, resolution, and status mutations, a later invocation re-reads and revalidates before the single bounded recovery retry. Timeout, network, HTTP 5xx, and lost-response failures remain ambiguous and are never replayed merely because an immediate read still shows the old state. Private-note writes have no approved automatic recovery retry: the continuation performs bounded exact private-note fingerprint reads, completes only when the note is observed, and otherwise terminates with `humanReconciliationRequired=true`. A note dedupe read that is rate limited remains a safe pre-write retry; a rate limit or transport failure after `createTicketNote` has started remains an ambiguous write and never authorizes a second note submission.

Approved private-note bodies are encrypted with AES-GCM under key material derived only from the dedicated SUPEROPS_PRIVATE_NOTE_ENCRYPTION_KEY Cloudflare secret and are atomically persisted outside the public operation record. The encryption key must be configured when durable private-note recovery is enabled and must remain distinct from SUPEROPS_INTERNAL_CONTINUATION_TOKEN, which is reserved for internal continuation authentication and service calls. Recovery requires the matching owner, item, and canonical note fingerprint. Public notes are never eligible for recovery, and note plaintext is excluded from operation status, compact results, diagnostics, logs, audit data, and errors.
...
Resolution fallback or retry is permitted only after a reliable response conclusively proves non-acceptance and stale checks still pass. After a timeout, network failure, HTTP 5xx, or lost response, the adapter reads current state first: a visible target completes without replay, changed `updatedTime` produces a stale outcome, and an unchanged or inconclusive state remains unresolved ambiguity. An unchanged immediate read alone never authorises a second mutation.
