...
function buildAgentInput(triggerId, scope, attempt = 1, resultCallbackEnabled = false) {
  ...
  if (scope.mode === "new-email-tickets") {
    return [
      ...
      'V2 first-pass gate: use policyMode "email-new-calls-v2". During the current-triage-first stabilization stage, do not call Ticket History MCP, Supabase, or any history/similarity/recurrence/emerging search. Set every required historyAssessment dimension to unknown and resultState to degraded; omit all history/emerging note sections and never imply history was checked or that no history exists. Complete bounded current-ticket evidence and the single safe apply without delaying for history; preserve exact-window, identity, content, stale-write, client, deduplication, verification, and no-replay safeguards.',
      "Email routing rule: this is a customer-email intake workflow. Any ticket that needs a human or engineer reply, clarification, investigation, or follow-up must remain in New Calls: use policyDisposition customer_request, manual_intake, or engineer_review as appropriate, but always use action leave and omit target.status. Never use action update or set Awaiting Engineer in email-new-calls-v2. Only a conclusively no-action item may use resolve_no_action with action resolve.",
